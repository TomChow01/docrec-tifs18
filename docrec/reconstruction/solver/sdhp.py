...
def shortest_directed_hamiltonian_path(
    matrix, start=None, end=None, precision=3, seed=0, id_=None
):
    ...
    solver = ConcordeATSPSolver(max_precision=precision, seed=seed, id_=id_)
    solution, _ = solver.solve(matrix_).solution()
    if solution is None:
...
{matrix_str}EOF'''
    ...
    def __init__(self, max_precision=3, seed=None, id_=None):
        ''' Class constructor. '''
...
#                warn('WARNING: Concorde can only handle distances < 2^31. Reducing precision to %d' % precision)
        ...
        tsp_filename = '/tmp/tsp{}.tsp'.format(self._id) if self._id is not None else '/tmp/tsp.tsp'
